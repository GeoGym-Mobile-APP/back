...
public interface WayRepository extends JpaRepository<Way, Long> {
    private static double rand(){
        ...
        return  random.nextDouble(100000)+1236;
    }
    @Query(nativeQuery = true,
            value = "WITH start_node AS (" +
                    ...
                    "    FROM pgr_dijkstra(" +
                    "        'SELECT gid AS id, source, target,   (cost * random()::numeric * (1000000 - 1000) + 1000) as cost, x1, y1, x2, y2 FROM ways'," +
    ...
    @Query(value =
            "WITH start_point AS (" +
                    ...
                    "        'SELECT gid AS id, source, target, (cost + 1246.54) * 15153.1245 as cost, x1, y1, x2, y2 FROM ways'," +
   ...
   *  @Query(value =
            "WITH start_node AS (" +
                    ...
                    "        'SELECT gid AS id, source, target, cost AS cost, x1, y1, x2, y2 FROM ways'," +
                    "        (SELECT id FROM start_node)," +
                    "        (SELECT id FROM end_node)" +
                    "    )" +
                    ")" +
                    ...
                    "JOIN ways_vertices_pgr wvp ON sp.node = wvp.id",
            nativeQuery = true)
          List<Object[]> findAlterbativeShortestPath(
            @Param("startLon") Double startLon,
            @Param("startLat") Double startLat,
            @Param("endLon") Double endLon,
            @Param("endLat") Double endLat
